...
static const char *s_tag = "usrButtonPasswordPage";
...
typedef struct {
    ...
    button_password_callback_t success_callback;
    button_password_callback_t cancel_callback;
    bool is_initialized;
    bool is_visible;
    bool keyboard_visible;

    int attempt_count;
} usrButtonPasswordPage_t;

static usrButtonPasswordPage_t passwordPage = {0};
...
void usrButtonPasswordPage_init(lv_obj_t *parent)
{
    if (passwordPage.is_initialized) {
        ESP_LOGW(s_tag, "Button password page already initialized");
        return;
    }
    ...
    lv_obj_set_style_bg_color(passwordPage.modal_bg, color_background, 0);  // Tamamen siyah
    lv_obj_set_style_bg_opa(passwordPage.modal_bg, LV_OPA_90, 0);  // %90 opaklık
    ...
    lv_obj_set_style_text_color(passwordPage.title_label, color_primary, 0);
    ...
    lv_label_set_text(passwordPage.instruction_label, "Enter your password to access button settings");
    lv_obj_set_style_text_font(passwordPage.instruction_label, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(passwordPage.instruction_label, color_secondary, 0);
...
void usrButtonPasswordPage_show(button_password_callback_t success_callback, button_password_callback_t cancel_callback)
{
    ...
    ESP_LOGI(s_tag, "Button password page shown on top layer");
}
...
void usrButtonPasswordPage_hide(void)
{
    ...
    ESP_LOGI(s_tag, "Button password page hidden");
}
...
void usrButtonPasswordPage_destroy(void)
{
    ...
    memset(&passwordPage, 0, sizeof(usrButtonPasswordPage_t));

    ESP_LOGI(s_tag, "Button password page destroyed");
}
