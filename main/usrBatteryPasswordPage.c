...
static const char *s_tag = "usrBatteryPasswordPage";
...
typedef struct {
    ...
    battery_password_callback_t success_callback;
    ...
    int attempt_count;
} usrBatteryPasswordPage_t;

static usrBatteryPasswordPage_t passwordPage = {0};
...
void usrBatteryPasswordPage_init(lv_obj_t *parent)
{
    if (passwordPage.is_initialized) {
        ESP_LOGW(s_tag, "Battery password page already initialized");
        return;
    }

    // Create modal background - kurumsal siyah arka plan
    passwordPage.modal_bg = lv_obj_create(lv_scr_act());
    lv_obj_set_size(passwordPage.modal_bg, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_pos(passwordPage.modal_bg, 0, 0);
    lv_obj_set_style_bg_color(passwordPage.modal_bg, color_background, 0);  // Kurumsal siyah
    ...
    // Create main container - kurumsal tema
    passwordPage.page_container = lv_obj_create(passwordPage.modal_bg);
    lv_obj_set_size(passwordPage.page_container, 400, 320);
    lv_obj_center(passwordPage.page_container);
    lv_obj_set_style_bg_color(passwordPage.page_container, color_background, 0);  // Ana arka plan rengi
    lv_obj_set_style_border_color(passwordPage.page_container, color_primary, 0);  // Primary renk border
    lv_obj_set_style_border_width(passwordPage.page_container, 3, 0);  // Kalın border
    lv_obj_set_style_radius(passwordPage.page_container, 12, 0);  // Rounded corners
    ...
    // Title label - kurumsal primary renk
    ...
    lv_obj_set_style_text_color(passwordPage.title_label, color_primary, 0);  // Primary renk
    lv_obj_align(passwordPage.title_label, LV_ALIGN_TOP_MID, 0, 10);

    // Instruction label - secondary renk
    passwordPage.instruction_label = lv_label_create(passwordPage.page_container);
    lv_label_set_text(passwordPage.instruction_label, "Enter your password to access battery settings");
    lv_obj_set_style_text_font(passwordPage.instruction_label, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(passwordPage.instruction_label, color_secondary, 0);  // Secondary renk
    ...
    // Password input textarea - kurumsal tema
    ...
    // Style textarea - kurumsal renkler
    lv_obj_set_style_bg_color(passwordPage.password_textarea, color_dark, 0);  // Dark background
    lv_obj_set_style_text_color(passwordPage.password_textarea, color_secondary, 0);  // Light text
    lv_obj_set_style_border_color(passwordPage.password_textarea, color_primary, 0);  // Primary border
    lv_obj_set_style_border_width(passwordPage.password_textarea, 2, 0);
    lv_obj_set_style_radius(passwordPage.password_textarea, 8, 0);
    lv_obj_set_style_pad_all(passwordPage.password_textarea, 10, 0);
    ...
    // Confirm button - kurumsal primary renk
    ...
    lv_obj_set_style_bg_color(passwordPage.confirm_btn, color_primary, 0);  // Primary renk
    lv_obj_set_style_radius(passwordPage.confirm_btn, 8, 0);
    lv_obj_set_style_border_color(passwordPage.confirm_btn, color_secondary, 0);
    lv_obj_set_style_border_width(passwordPage.confirm_btn, 1, 0);
    ...
    lv_obj_set_style_text_color(confirm_label, color_dark, 0);  // Dark text on primary
    lv_obj_center(confirm_label);

    // Cancel button - dark tema
    ...
    lv_obj_set_style_bg_color(passwordPage.cancel_btn, color_dark, 0);  // Dark background
    lv_obj_set_style_radius(passwordPage.cancel_btn, 8, 0);
    lv_obj_set_style_border_color(passwordPage.cancel_btn, color_primary, 0);
    lv_obj_set_style_border_width(passwordPage.cancel_btn, 2, 0);
    ...
    lv_obj_set_style_text_color(cancel_label, color_secondary, 0);  // Light text on dark
    lv_obj_center(cancel_label);

    // Create keyboard - kurumsal tema
    ...
    // Style keyboard - kurumsal renkler
    lv_obj_set_style_bg_color(passwordPage.keyboard, color_dark, 0);
    lv_obj_set_style_border_color(passwordPage.keyboard, color_primary, 0);
    lv_obj_set_style_border_width(passwordPage.keyboard, 2, 0);
    lv_obj_set_style_radius(passwordPage.keyboard, 10, 0);
    ...
    ESP_LOGI(s_tag, "Battery password page initialized with corporate theme");
}

void usrBatteryPasswordPage_show(battery_password_callback_t success_callback, battery_password_callback_t cancel_callback)
{
    if (!passwordPage.is_initialized) {
        ESP_LOGW(s_tag, "Battery password page not initialized");
        return;
    }
    ...
    ESP_LOGI(s_tag, "Battery password page shown on top layer");
}
...
void usrBatteryPasswordPage_hide(void)
{
    ...
    ESP_LOGI(s_tag, "Battery password page hidden");
}
...
void usrBatteryPasswordPage_destroy(void)
{
    ...
    memset(&passwordPage, 0, sizeof(usrBatteryPasswordPage_t));

    ESP_LOGI(s_tag, "Battery password page destroyed");
}
